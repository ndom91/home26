...
export function ScreenshotLink({
  ...
  return (
    ...
    >
      ...
      <span
        aria-hidden="true"
        className="not-prose pointer-events-none invisible absolute bottom-full left-1/2 z-30 mb-3 w-80 max-w-[min(20rem,calc(100vw-2rem))] -translate-x-1/2 translate-y-1 scale-[0.98] overflow-hidden rounded-2xl border border-blog-rule bg-blog-panel p-2 font-normal opacity-0 shadow-2xl shadow-black/35 transition-[opacity,scale,translate,visibility] duration-200 ease-out before:absolute before:-bottom-3 before:left-0 before:h-3 before:w-full before:content-[''] group-hover/screenshot:visible group-hover/screenshot:translate-y-0 group-hover/screenshot:scale-100 group-hover/screenshot:opacity-100 group-focus-within/screenshot:visible group-focus-within/screenshot:translate-y-0 group-focus-within/screenshot:scale-100 group-focus-within/screenshot:opacity-100 motion-reduce:transition-none"
      >
        {previewFailed ? (
          <span className="image-outline flex aspect-video w-full items-center justify-center rounded-lg bg-blog-bg px-4 text-center font-mono text-[10px] uppercase tracking-widest text-blog-muted">
            Preview unavailable
          </span>
          ...
          <img
            ...
            className="image-outline m-0! block aspect-video w-full! max-w-full! rounded-lg object-cover object-center"
          />
        ) : (
          <span className="image-outline flex aspect-video w-full items-center justify-center rounded-lg bg-blog-bg px-4 text-center font-mono text-[10px] uppercase tracking-widest text-blog-muted">
            Loading…

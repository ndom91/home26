...
const homeBarClass =
  'relative z-10 grid min-h-16 grid-cols-[auto_1fr] items-center border-b border-rule bg-paper text-[0.7rem] uppercase tracking-[0.16em] text-ink max-[820px]:grid-cols-[minmax(0,1fr)_auto] max-[520px]:text-[0.72rem] max-[520px]:tracking-[0.17em]'
...
export function SiteHeader() {
  ...
  return (
    <header className={homeBarClass}>
      <nav
        className={`${homeBarCellClass} flex min-w-0 flex-wrap justify-center gap-x-5 gap-y-2 max-[820px]:justify-start max-[520px]:gap-x-4 [&>a]:relative [&>a]:inline-flex [&>a]:items-center [&>a]:transition-colors [&>a]:before:absolute [&>a]:before:inset-x-[-0.35rem] [&>a]:before:inset-y-[-0.75rem] [&>a]:before:content-[''] [&>a]:hover:text-accent`}
        aria-label="Primary"
      >
        <Link to="/">Home</Link>
      ...
      <button
        className="group me-5 inline-grid min-h-10 min-w-10 cursor-pointer place-items-center justify-self-end rounded-full border-0 bg-transparent p-0 text-inherit outline-none transition-transform active:scale-[0.96] [view-transition-name:disabled] motion-reduce:transition-none max-[520px]:me-4"
        type="button"
        onClick={toggleTheme}

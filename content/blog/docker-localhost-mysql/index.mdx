...
category: "linux"
standardSiteUri: at://did:plc:nmwj6xne7mdpkry5pzvfendv/site.standard.document/docker-localhost-mysql
cover:
  imageFile: "./cover.png"
---

I was having the hardest time getting applications in docker containers to connect to the instance of mysql runnning on the host without using host networking mode in docker. So I figured I'd share my solution here for others.

The idea is to simply enable some local routing rules and use `iptables` to route traffic from Docker's subnet to the localhost's port 3306 for MySQL.
...
At this point, we should be able to route on our local machine on port `3306`. That is, from the docker subnet's gateway (`172.17.0.1` aka the localhost) to our host's loopback address where the MySQL instance is listening (`127.0.0.1:3306`).

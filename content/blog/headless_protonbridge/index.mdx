...
category: "linux"
discussionAtprotoUri: at://did:plc:nmwj6xne7mdpkry5pzvfendv/app.bsky.feed.post/3mncgstcohk26
standardSiteUri: at://did:plc:nmwj6xne7mdpkry5pzvfendv/site.standard.document/headless_protonbridge
cover:
  imageFile: "./cover.png"
---
...
I'll be doing this on an Ubuntu 24.04 server, so we'll need to install some dependencies.

1. `sudo apt install libfido2-dev libcbor-dev libssl-dev pkg-config`
2. Install golang (the Debian repo one is usually too old - https://go.dev/doc/install)
...
git checkout v3.20.1   # or whatever the latest tag is
```
...
```bash filename="/etc/systemd/system/proton-bridge.service"
[Unit]

...
standardSiteUri: at://did:plc:nmwj6xne7mdpkry5pzvfendv/site.standard.document/doh
cover:
  imageFile: "./cover.png"
---
...
flowchart LR
  Client["Client"]
  subgraph host["Proxmox Host · public IP"]
    NGINX["NGINX<br/>dns.ndo.dev<br/>:443 · TLS terminate"]
  end
  subgraph vm["DNS VM · 192.168.1.2"]
    direction TB
    DOH["dns-over-https<br/>:8083"]
    UNBOUND["Unbound resolver<br/>127.0.0.1:5083"]
  end
  Client -->|"HTTPS /dns-query"| NGINX
  NGINX -->|"proxy → :8083"| DOH
  DOH -->|":8083 → :5083"| UNBOUND
```
